/**
 * SLL class'ý kitaptan alindi!
 * reverseToString method and reversing method
 * are created and imlemented by GozdeDogan, 7 Mart 2017, 18:05
 */
...
// Insert solution to programming exercise 1, section 5, chapter 2 here
	...
	/**
	 * SingleLinkedList node'larini ters bir sekilde yaziyor.
	 * reversing metodu ilk elemanla cagrildi!
	 * reversing metodu recursive bir metot.
	 */
    public String reverseToString(){
        StringBuilder list = new StringBuilder("reverseMyList: [");
        if(head != null)
            reversing(head, 0, list);
        list.append("]");
	...
	/**
	 * SingleLinkedList node'larini ters bir sekilde 
	 * yazilmasi icin yazilan recursive metot.
	 */
    private void reversing(Node p, int index, StringBuilder list){
        if(index == size()-1 && p != null){
            list.append(p.data.toString());
        }
        else if(p.next != null){
            reversing(p.next, 1, list);
            list.append(p.data.toString());
            if(index!=0){
                list.append(" ==> ");
            }
